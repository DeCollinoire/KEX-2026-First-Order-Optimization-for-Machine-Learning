...
def setupOptimizerList(lossObj, initPos):
    ...
    # SGD setup
    SGDLrList = [0.01, 0.02, 0.03, 0.1, 3.5, 1800]
    for lr in SGDLrList:
        optSGDLrList.append(sgd.SGD(lossObj, initPos, lr=lr))
    optSGDList.append(optSGDLrList)

    # Momentum setup
    momentumLrList = [0.01, 0.02, 0.03, 0.1]
    momentumDfList = [0.1, 0.2, 0.5, 0.9]
    ...
    for df in momentumDfList:
        optMomentumDfList.append(momentum.Momentum(lossObj, initPos, learningRate=0.001, decayFactor=df))
    optMomentumList.append(optMomentumLrList)
    optMomentumList.append(optMomentumDfList)

    # Nesterov setup
    nesterovLrList = [0.01, 0.02, 0.03, 0.1]
    nesterovDfList = [0.1, 0.2, 0.5, 0.9]
    ...
    for df in nesterovDfList:
        optNesterovDfList.append(nesterov.Nesterov(lossObj, initPos, lr=0.001, decayFactor=df))
    optNesterovList.append(optNesterovLrList)
    optNesterovList.append(optNesterovDfList)

    # Adam setup
    adamBaseCaselr = 0.002
    adamBaseCaseForgettingfactorM = 0.9
    adamBaseCaseForgettingfactorR = 0.999
    adamLrList = [0.001, 0.002,0.003, 0.01]
    adamFfMList = [0.2, 0.8, 0.9, 0.91]
    adamFfRList = [0.99, 0.999]
...
def main():
    ...
    lossObj, initPos = setupProblem(problemName=problemName, dim=10, datasetFilepath=datasetFilepath,
                                    ...
                                    batchSize=1000000, toDense=False)  # QDF, Rosenbrock; datasetFilepath is only needed for LogReg
    ...
    train(allOpts, lossObj=lossObj,nrEpochs=50)
